...
%emw24Jan -- below is good, can you also explain the direction of relationship in europe? Do events get later/earlier with higher/lower latitude and MAT? % alina_jan27 -- I believe I described that in later paragraphs. Would you like me to move it up? EMW replies -- yes, I think we should keep the overall effect and the directionality together when possible. Not critical to organize now though. 
Overall, our models show that spring events are not related to provenance latitude or mean annual temperature in North America, and only weakly related in Europe (Fig. \ref{figure:springfall_latmat} \& Supplement Table. \ref{supplement-table:model_sf_lat}). \unsure{Should I add climate overlap plots here too?} %emw24Jan -- do you mean add a table of climate overlap to the supp? % alina_jan27 -- I meant if the plots where I plotted climate overlap percentage against DOY should be included here too, so easier for people to compare perhaps? EMW replies -- sounds like something to put in the supp, I think. 
