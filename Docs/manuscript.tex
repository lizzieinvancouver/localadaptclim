...
In contrast, fall events (budset, leaf senescence, leaf abcission) advanced strongly with provenance latitude and mean annual temperature, meaning fall events are earlier where provenance mean annual temperature is lower. This relationship, however, was observed mostly in North America where fall events advance 4.2 (UI = 4) days per degree we move north, or 6.4 days (UI) per degree decline in MAT$^{\circ}$C (Fig. \ref{figure:continent}). In Europe, such relationship is weak: advance 0.5 (UI) days per degree we move north, or 0.7 days (UI) per degree decline in MAT $^{\circ}$C(Fig. \ref{figure:continent}). 
