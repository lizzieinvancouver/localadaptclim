...
While we expected that coarse metrics, such as latitude and MAT, would generally represent how similar the climates are between the provenances and gardens, we also estimated climate overlap in months much closer to the events to further test how much climate similarity between provenances and gardens predicts provenance effects (i.e. differential responses observed among plant populations from different geographical origins). For spring events, we considered overlap across March to May. However, results were not qualitatively different than using MAT (See Fig.S\ref{supplement-figure:overlap_scatterplot} in Supporting Information). We observed very weak effects of climate overlap on spring events (\Sexpr{format(round(abs(mean(all_species_springpost_fitC_spring_overlap)),digits = 2), nsmall=2)} [\Sexpr{format(round(abs(quantile(all_species_springpost_fitC_spring_overlap, 0.05)), digits = 2),nsmall=2)} - \Sexpr{format(round(abs(quantile(all_species_springpost_fitC_spring_overlap, 0.95)), digits = 2),nsmall=2)}] days per one percent increase in climate overlap), nearly identical across angiosperms (\Sexpr{format(round(abs(mean(deciduous_springpost_fitC_spring_overlap)),digits = 2), nsmall=2)} [\Sexpr{format(round(abs(quantile(deciduous_springpost_fitC_spring_overlap, 0.95)), digits = 2),nsmall=2)} - \Sexpr{format(round(abs(quantile(deciduous_springpost_fitC_spring_overlap, 0.05)), digits = 2),nsmall=2)}]) and gymnosperms (\Sexpr{format(round(abs(mean(evergreens_springpost_fitC_spring_overlap)),digits = 2), nsmall=2)} [\Sexpr{format(round(abs(quantile(evergreens_springpost_fitC_spring_overlap, 0.05)), digits = 2),nsmall=2)} - \Sexpr{format(round(abs(quantile(evergreens_springpost_fitC_spring_overlap, 0.95)), digits = 2),nsmall=2)}]). Fall events advanced as climate overlap declined, but slightly more strongly for gymnosperms (advancing \Sexpr{format(round(abs(mean(evergreenspost_fitC_fall_overlap)),digits = 2), nsmall=2)} [\Sexpr{format(round(abs(quantile(evergreenspost_fitC_fall_overlap, 0.05)), digits = 2),nsmall=2)} - \Sexpr{format(round(abs(quantile(evergreenspost_fitC_fall_overlap, 0.95)), digits = 2),nsmall=2)}] days per one percent decline in climate overlap) (Fig.S\ref{supplement-figure:overlap_posterior} in Supporting Information).
...
\begin{figure}[!h] 
 ...
 \includegraphics[width=\textwidth]{..//..//localadaptclim/Docs/figure_ms/springfall_latmat.jpg}
    \caption{Event day of year (DOY) in relation to provenance latitude and MAT, coded by symbol for species and color for garden with linear fits from hierarchical Bayesian models. Spring events shown on top and fall events at the bottom. European gardens and species are made bold and denoted by an asterisk (*). } 
