...
Overall, spring events were not related to provenance latitude (\Sexpr{format(mean(northamerican_springpost_fitC_spring_lat),digits = 1)} [\Sexpr{format(quantile(northamerican_springpost_fitC_spring_lat, 0.05), digits = 1)}-\Sexpr{format(quantile(northamerican_springpost_fitC_spring_lat, 0.95), digits = 2)}]) or MAT (\Sexpr{format(mean(northamerican_springpost_fitC_spring_mat),digits = 1)} [\Sexpr{format(quantile(northamerican_springpost_fitC_spring_mat, 0.05), digits = 2)}-\Sexpr{format(quantile(northamerican_springpost_fitC_spring_mat, 0.95), digits = 2)}]) in North America. \todo{add figure here} Spring events advanced slightly with provenance latitude (\Sexpr{format(mean(european_springpost_fitC_spring_lat),digits = 1)} [\Sexpr{format(quantile(european_springpost_fitC_spring_lat, 0.05), digits = 2)}-\Sexpr{format(quantile(european_springpost_fitC_spring_lat, 0.95), digits = 2)}]) and MAT (\Sexpr{format(mean(european_springpost_fitC_spring_mat),digits = 1)} [\Sexpr{format(quantile(european_springpost_fitC_spring_mat, 0.05), digits = 2)}-\Sexpr{format(quantile(european_springpost_fitC_spring_mat, 0.95), digits = 2)}]) in Europe, meaning spring events are slightly earlier where provenance MAT is lower (i.e., higher, more northern latitudes)(Fig. \ref{figure:springfall_latmat} \& Supplement Table. \ref{supplement-table:model_sf_lat}), though uncertainty intervals strongly overlapped zero. Results were similar using other distance metrics in lieu of latitude (see Supplement Fig.\ref{supplement-figure:lat_distance} for results using the absolute value of the distance and the spherical distance difference between provenance and garden latitude). 
