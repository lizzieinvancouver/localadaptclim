...
%EMWApr20 -- please organize your first outline using \enumerate, which is much easier for me to follow and refer to #s then
...
%EMWApr20 -- table in supp and map in main text (figure caption should refer to table in supp)
\\
\\
...
%EMWApr20 -- if you look at the intervals the results in NA and Europe seem similar so I changed the text. Also -- you should report the OVERALL model estimate across both europe and NA ...
Overall, spring events were not related to provenance latitude or MAT, neither across continents (Fig. \ref{figure:springfall_latmat} \& Supplement Table. \ref{supplement-table:model_sf_lat}, ADD latitude: stats, MAT: stats), nor in North America (latitude: \Sexpr{format(round(mean(northamerican_springpost_fitC_spring_lat),digits = 2), nsmall=2)} days/degree [\Sexpr{format(quantile(northamerican_springpost_fitC_spring_lat, 0.05), digits = 1)} - \Sexpr{format(quantile(northamerican_springpost_fitC_spring_lat, 0.95), digits = 2)}]; MAT \Sexpr{format(mean(northamerican_springpost_fitC_spring_mat),digits = 1)} days/degree [\Sexpr{format(quantile(northamerican_springpost_fitC_spring_mat, 0.05), digits = 2)} - \Sexpr{format(quantile(northamerican_springpost_fitC_spring_mat, 0.95), digits = 2)}]) or Europe (latitude: \Sexpr{format(round(mean(european_springpost_fitC_spring_lat),digits = 2),nsmall=2)} days/degree [\Sexpr{format(quantile(european_springpost_fitC_spring_lat, 0.05), digits = 2)} - \Sexpr{format(quantile(european_springpost_fitC_spring_lat, 0.95), digits = 2)}]; MAT:\Sexpr{format(round(mean(european_springpost_fitC_spring_mat),digits = 2),nsmall=2)} days/degree [\Sexpr{format(quantile(european_springpost_fitC_spring_mat, 0.05), digits = 2)} - \Sexpr{format(quantile(european_springpost_fitC_spring_mat, 0.95), digits = 2)}]). Results were similar using other distance metrics in lieu of latitude (see Supplement Fig.\ref{supplement-figure:lat_distance} for results using the absolute value of the distance and the spherical distance difference between provenance and garden latitude). \\
\todo{add figure here} 
% Spring events advanced slightly with provenance latitude in Europe spring events are slightly earlier where provenance MAT is lower (i.e., higher, more northern latitudes), though uncertainty intervals strongly overlapped zero. 

%EMWApr20 -- reminder that Fig 1 needs a way to tell apart Europe and NA garden -- you can say in caption for now how the GardenID relates and eventually show on figure (add some text and a bracket to gardenID legend I think)
\begin{figure}[!h] 
    \centering
...
%EMWApr20 -- add stats for model (across Europe and NA) in first sentence. 
In contrast, fall events (budset, leaf senescence, leaf abcission) advanced strongly with provenance latitude and MAT, meaning fall events are earlier where provenance MAT is lower. This relationship, however, was observed mostly in North America where fall events advance \Sexpr{format(round(abs(mean(northamericanpost_fitC_fall_lat)),digits = 2),nsmall=2)} (\Sexpr{format(round(abs(quantile(northamericanpost_fitC_fall_lat, 0.95)), digits = 2),nsmall=2)} - \Sexpr{format(round(abs(quantile(northamericanpost_fitC_fall_lat, 0.05)), digits = 2),nsmall=2)}) days per degree northward, or \Sexpr{format(round(mean(northamericanpost_fitC_fall_mat),digits = 2),nsmall=2)} days (\Sexpr{format(round(abs(quantile(northamericanpost_fitC_fall_mat, 0.05)), digits = 2),nsmall=2)} - \Sexpr{format(round(abs(quantile(northamericanpost_fitC_fall_mat, 0.95)), digits = 2),nsmall=2)}) per degree decline in MAT ($^{\circ}$C) (Fig. \ref{figure:continent}), whereas in Europe these relationships were weaker: advance of \Sexpr{format(round(abs(mean(europeanpost_fitC_fall_lat)),digits = 2),nsmall=2)} (\Sexpr{format(round(abs(quantile(europeanpost_fitC_fall_lat, 0.95)), digits = 2),nsmall=2)} - \Sexpr{format(round(abs(quantile(europeanpost_fitC_fall_lat, 0.05)), digits = 2),nsmall=2)}) days per degree northward, or \Sexpr{format(round(mean(europeanpost_fitC_fall_mat),digits = 2),nsmall=2)} days (\Sexpr{format(round(abs(quantile(europeanpost_fitC_fall_mat, 0.05)), digits = 2),nsmall=2)} - \Sexpr{format(round(abs(quantile(europeanpost_fitC_fall_mat, 0.95)), digits = 2),nsmall=2)}) per degree decline in MAT $^{\circ}$C(Fig. \ref{figure:continent}). \\
...
\end{figure}
%EMWApr20 -- please fix all figure edits and add captions for next draft so we can finalize all the tweaks needed for submitting to journal

%EMWApr20 -- add the stats in below paragraph also. Annoying I know, but we're almost done!
Effects of provenance latitude on both spring and fall events were similar across angiosperms and gymnosperms. However, effects of MAT on spring events weakly diverged: spring events shifted earlier as MAT increases in angiosperms and delay as MAT increases in gymnosperms, except for \emph{Pseudotsuga menziesii}. Fall events delay in warmer locations for both species types, but slightly more so for gymnosperms (\Sexpr{format(round(mean(deciduouspost_fitC_fall_mat),digits = 2),nsmall=2)} days VS. \Sexpr{format(round(mean(evergreenspost_fitC_fall_mat),digits = 2),nsmall=2)} days)(Fig. \ref{figure:spp_type}).\\
...
We also tested whether climate effects on events were stronger when using metrics from seasonal windows closer to event dates, but results were not qualitatively different than using MAT (See supp? Not sure what results we mean here -- overlap?). We observed very weak effects of climate overlap on spring events, nearly identical across angiosperms and gymnosperms. Fall events diverge as climate overlap declines for both angiosperms and gymnosperms, but slightly more strongly for gymnosperms (Fig.\ref{figure:overlap}).
...
%EMWApr20 -- update figures with a) Spring events and continent ... b) Fall etc. so they look like ms figures. You also need to add the correct x axis -- they are likely days (spring or fall) per degree latitude or per degree MAT (C) ...
\begin{figure}[!h] 
    \centering
