...
\todo[inline]{add a table documenting studies attributes} %emw6Mar -- Tables should go in supplement. But we should add estimates to statements we make throughout. These should be added as mean +/- 90% unvertainty (posterior) intervals (UI) parenthetically using /Sexpr so they update automatically. 
Overall, spring events were not related to provenance latitude or mean annual temperature in North America (insert the mean slope here, followed by the 90\% intervals ), and only weakly related in Europe (insert slope and UI), where springs events are slightly earlier where provenance mean annual temperature is lower (i.e., higher, more northern latitudes)(Fig. \ref{figure:springfall_latmat} \& Supplement Table. \ref{supplement-table:model_sf_lat}). Results were similar using other distance metrics in lieu of latitude (see Supplement Fig.\ref{supplement-figure:lat_distance} for results using the absolute value of the distance and the spherical distance difference between provenance and garden latitude). %emw6Mar -- this text from below -- "advanced strongly with provenance latitude and mean annual temperature, meaning fall events are earlier where provenance mean annual temperature is lower" is good, can you copy that phrasing here instead "where springs events are slightly earlier where provenance mean annual temperature is lower"? I think it would be something like 'delay slightly ...' 
...
In contrast, fall events (budset, leaf senescence, leaf abcission) advanced strongly with provenance latitude and mean annual temperature, meaning fall events are earlier where provenance mean annual temperature is lower (i.e., higher, more northern latitudes). This relationship, however, was observed mostly in North America where fall events advance 4.2 days per degree (UI), or 6.4 days (UI) per degree decline in MAT$^{\circ}$C (Fig. \ref{figure:continent}). In Europe, such relationship is weak: advance 0.5 days (UI) per degree we move north, or 0.6 days (UI) per degree decline in MAT $^{\circ}$C(Fig. \ref{figure:continent}). %emw6Mar -- above numbers (e.g., 4.2 should be done with /Sexpr and include UI in parentheses)
