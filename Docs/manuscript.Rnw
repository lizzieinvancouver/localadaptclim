...
Many of the currently observed responses to climate change appear to be mostly plastic (CITES), including the most reported biological response to climate change---shifting phenology. Phenology (timing of recurring seasonal events) governs the timing of transitions between dormancy and active growth for many organisms, allowing them to time reproduction and fully exploit the resources of each growing season \citep{chuine10,hanninen11}. % add some animal refs -- Chuine Annual Review would be good: Process-Based Models of Phenology for Plants and Animals
As such, phenology plays a significant role in determining fitness (CITES). Shifted phenology in recent decades---with many events moving several days per decade (CITES)---has thus led to concerns about fitness consequences, and the limits of possible future shifts. Future phenological shifts will depend on how much phenology is determined by adaptation versus plasticity will determine, yet our understanding of the balance of these two approaches to variation is limited, even for species groups that are critical to both forecasting and have been well studied, such as trees. 

Tree phenology is critically important to climate change forecasting and has been an area of long-term study of adaptation and plasticity. The timing of budburst and senescence can impact plant competition \citep{fridley12}, growth \citep{myneni97}, and carbon sequestration \citep{Barichivich12}---thus effecting climate change itself. Hundreds of years of study have established common clines in fall phenology, as locations with shorter growing season (poleward, higher elevation) exhibit earlier stopping of growth (e.g., via budset), and connected these to an underlying proximate mechanism of changing photoperiod cues, driven by local adaptation to the local growing season and relevant photoperiod to stop growth (CITES). In contrast, spring phenology appears far more plastic (CITES) and determined more strongly by temperature (CITES). Many studies, however, have argued that spring phenology shows important levels of adaptation. 

Evidence for local adaptation in spring phenology comes mostly from common garden studies. For more than 250 years, researchers have been comparing populations of trees from different geographical origins under the same environmental conditions through these experiments to disentangle the effects of environmental and genetic variation on trees’ phenology and phenotype \citep{AitkenBemmels16, Alberto13}. Studies have documented provenance differences of XX-XX days per (degree C or latitude??) for some species (\emph{Populus} and \emph{Fagus}?) where?? while others have not found any trends in budburst (give species and where). This has led to some debate over the prevalence and importance of adaptation in spring tree phenology. While clines of spring phenology have been found in studies in both Europe (CITES) and North America (CITES) they appear potentially more common in European studies. 

Continental differences in patterns of adaptation versus plasticity could be driven by climatic differences, as North American springs are more variable across years than European ones (CITES). Such high temporal variability means that distant sites can effectively experience the same spring climate, but in different years. Studies of spring phenology in arboreta suggest cues for budburst may vary depending on continental climate (CITES), but are poorly controlled compared to traditional provenance trials, making them difficult to use for tests of plasticity versus adaptation. Even for more carefully designed common garden differences in study design---especially studied species---may complicate understanding what underlies potential trends across continents, with European studies focusing more on deciduous angiosperms and North American studies focusing often on conifers.  
% Alberto shows elevation cline 
% Soolananayakanahally et al 2013 finds weak cline ... I think there should also be Fagus studies. Vitasse has some elevational studies... 
...
To address these questions, we combined a new meta-analysis of all common garden experiments in temperate tree species across Europe and North America reporting spring phenology with Bayesian hierarchical models. 
 ...
 %REFS
 % https://link.springer.com/article/10.1007/s00484-014-0900-y
 % emw could not open bibliography_local_adaptation.bib so just adding my refs as separate file for now. 
 % kharouba2018 for estimates of change across species per decade (could go with your menzel ref well)
 % \cite{Cleland:2007or} classic on phenology and why it matters with climate change, carbon etc. -- easy read so I recommend it!
 % \cite{chuinearees} covers both insect and plant phenology and discusses plasticity and adaptation
 % bonamour2019 -- good paper on plasticity (whole issue of this journal was good) 
 % savolainenAREES -- have not read recently but sounds relevant
 % gauzere2020 -- cite and read
 % \cite{ospreebbms,ospreephoto} -- relevant papers from the lab
 % \cite{zohner2017} cover variation in spring climate across continents
 % \cite{chang2021} and \cite{chuine2016} -- relevant for discussion of chilling
 % See also Salk 2020 paper and some papers mentioned above
...
Spring events slightly delayed \Sexpr{format(round(abs(mean(deciduous_springpost_fitC_spring_lat)),digits = 2), nsmall=2)} (\Sexpr{format(round(abs(quantile(deciduous_springpost_fitC_spring_lat, 0.05)), digits = 2),nsmall=2)} - \Sexpr{format(round(abs(quantile(deciduous_springpost_fitC_spring_lat, 0.95)), digits = 2),nsmall=2)}) days per degree north in angiosperms and \Sexpr{format(round(abs(mean(evergreens_springpost_fitC_spring_lat)),digits = 2), nsmall=2)} (\Sexpr{format(round(abs(quantile(evergreens_springpost_fitC_spring_lat, 0.95)), digits = 2),nsmall=2)} - \Sexpr{format(round(abs(quantile(evergreens_springpost_fitC_spring_lat, 0.05)), digits = 2),nsmall=2)}) days per degree north in gymnosperms. Fall events advanced \Sexpr{format(round(abs(mean(deciduouspost_fitC_fall_lat)),digits = 2), nsmall=2)} (\Sexpr{format(round(abs(quantile(deciduouspost_fitC_fall_lat, 0.95)), digits = 2),nsmall=2)} - \Sexpr{format(round(abs(quantile(deciduouspost_fitC_fall_lat, 0.05)), digits = 2),nsmall=2)}) days per degree north in angiosperms and \Sexpr{format(round(abs(mean(evergreenspost_fitC_fall_lat)),digits = 2), nsmall=2)} (\Sexpr{format(round(abs(quantile(evergreenspost_fitC_fall_lat, 0.95)), digits = 2),nsmall=2)}-\Sexpr{format(round(abs(quantile(evergreenspost_fitC_fall_lat, 0.05)), digits = 2),nsmall=2)}) days per degree north in gymnosperms.
However, effects of MAT on spring events weakly diverged (Fig.\ref{figure:continental_spptype_effect}B). Spring events advanced \Sexpr{format(round(abs(mean(deciduous_springpost_fitC_spring_mat)),digits = 2), nsmall=2)} (\Sexpr{format(round(abs(quantile(deciduous_springpost_fitC_spring_mat, 0.95)), digits = 2),nsmall=2)} - \Sexpr{format(round(abs(quantile(deciduous_springpost_fitC_spring_mat, 0.05)), digits = 2),nsmall=2)}) days/$^{\circ}$C as MAT increased in angiosperms and delayed \Sexpr{format(round(abs(mean(evergreens_springpost_fitC_spring_mat)),digits = 2), nsmall=2)} (\Sexpr{format(round(abs(quantile(evergreens_springpost_fitC_spring_mat, 0.05)), digits = 2),nsmall=2)} - \Sexpr{format(round(abs(quantile(evergreens_springpost_fitC_spring_mat, 0.95)), digits = 2),nsmall=2)}) days/$^{\circ}$C as MAT increased in gymnosperms. Fall events delayed in warmer locations for both species types, but slightly more so for gymnosperms (\Sexpr{format(round(mean(evergreenspost_fitC_fall_mat),digits = 2),nsmall=2)} days) than angiosperms (\Sexpr{format(round(mean(deciduouspost_fitC_fall_mat),digits = 2),nsmall=2)} days) (Fig. \ref{figure:continental_spptype_effect}B).\\
...
While we believe that coarse metrics such as latitude and MAT would ultimately represent how similar the climates are between the provenances and gardens, we also estimated climate overlap in months that matter for the events to further test how much climate similarity between provenances and gardens predicts local adaptation. For spring events, we considered overlap across March to May. However, results were not qualitatively different than using MAT (See Fig. \ref{supplement-figure:overlap_scatterplot} in Supplements). We observed very weak effects of climate overlap on spring events (\Sexpr{format(round(abs(mean(all_species_springpost_fitC_spring_overlap)),digits = 2), nsmall=2)} [\Sexpr{format(round(abs(quantile(all_species_springpost_fitC_spring_overlap, 0.05)), digits = 2),nsmall=2)} - \Sexpr{format(round(abs(quantile(all_species_springpost_fitC_spring_overlap, 0.95)), digits = 2),nsmall=2)}] days per one per cent increase in climate overlap), nearly identical across angiosperms (\Sexpr{format(round(abs(mean(deciduous_springpost_fitC_spring_overlap)),digits = 2), nsmall=2)} [\Sexpr{format(round(abs(quantile(deciduous_springpost_fitC_spring_overlap, 0.95)), digits = 2),nsmall=2)} - \Sexpr{format(round(abs(quantile(deciduous_springpost_fitC_spring_overlap, 0.05)), digits = 2),nsmall=2)}]) and gymnosperms (\Sexpr{format(round(abs(mean(evergreens_springpost_fitC_spring_overlap)),digits = 2), nsmall=2)} [\Sexpr{format(round(abs(quantile(evergreens_springpost_fitC_spring_overlap, 0.05)), digits = 2),nsmall=2)} - \Sexpr{format(round(abs(quantile(evergreens_springpost_fitC_spring_overlap, 0.95)), digits = 2),nsmall=2)}]). Fall events advanced as climate overlap declined, but slightly more strongly for gymnosperms (advancing \Sexpr{format(round(abs(mean(evergreenspost_fitC_fall_overlap)),digits = 2), nsmall=2)} [\Sexpr{format(round(abs(quantile(evergreenspost_fitC_fall_overlap, 0.05)), digits = 2),nsmall=2)} - \Sexpr{format(round(abs(quantile(evergreenspost_fitC_fall_overlap, 0.95)), digits = 2),nsmall=2)}] days per one per cent decline in climate overlap) (Fig.\ref{supplement-figure:overlap_posterior} in Supplements).
