...
Overall, spring events were not related to provenance latitude (\Sexpr{format(round(mean(northamerican_springpost_fitC_spring_lat),digits = 2), nsmall=2)} days/degree [\Sexpr{format(quantile(northamerican_springpost_fitC_spring_lat, 0.05), digits = 1)} - \Sexpr{format(quantile(northamerican_springpost_fitC_spring_lat, 0.95), digits = 2)}]) or MAT (\Sexpr{format(mean(northamerican_springpost_fitC_spring_mat),digits = 1)} days/degree [\Sexpr{format(quantile(northamerican_springpost_fitC_spring_mat, 0.05), digits = 2)} - \Sexpr{format(quantile(northamerican_springpost_fitC_spring_mat, 0.95), digits = 2)}]) in North America. \todo{add figure here} Spring events advanced slightly with provenance latitude (\Sexpr{format(round(mean(european_springpost_fitC_spring_lat),digits = 2),nsmall=2)} days/degree [\Sexpr{format(quantile(european_springpost_fitC_spring_lat, 0.05), digits = 2)} - \Sexpr{format(quantile(european_springpost_fitC_spring_lat, 0.95), digits = 2)}]) and MAT (\Sexpr{format(round(mean(european_springpost_fitC_spring_mat),digits = 2),nsmall=2)} days/degree [\Sexpr{format(quantile(european_springpost_fitC_spring_mat, 0.05), digits = 2)} - \Sexpr{format(quantile(european_springpost_fitC_spring_mat, 0.95), digits = 2)}]) in Europe, meaning spring events are slightly earlier where provenance MAT is lower (i.e., higher, more northern latitudes)(Fig. \ref{figure:springfall_latmat} \& Supplement Table. \ref{supplement-table:model_sf_lat}), though uncertainty intervals strongly overlapped zero. Results were similar using other distance metrics in lieu of latitude (see Supplement Fig.\ref{supplement-figure:lat_distance} for results using the absolute value of the distance and the spherical distance difference between provenance and garden latitude). 
...
In contrast, fall events (budset, leaf senescence, leaf abcission) advanced strongly with provenance latitude and MAT, meaning fall events are earlier where provenance MAT is lower. This relationship, however, was observed mostly in North America where fall events advance \Sexpr{format(round(abs(mean(northamericanpost_fitC_fall_lat)),digits = 2),nsmall=2)} (\Sexpr{format(round(abs(quantile(northamericanpost_fitC_fall_lat, 0.95)), digits = 2),nsmall=2)} - \Sexpr{format(round(abs(quantile(northamericanpost_fitC_fall_lat, 0.05)), digits = 2),nsmall=2)}) days per degree northward, or \Sexpr{format(round(mean(northamericanpost_fitC_fall_mat),digits = 2),nsmall=2)} days (\Sexpr{format(round(abs(quantile(northamericanpost_fitC_fall_mat, 0.05)), digits = 2),nsmall=2)} - \Sexpr{format(round(abs(quantile(northamericanpost_fitC_fall_mat, 0.95)), digits = 2),nsmall=2)}) per degree decline in MAT$^{\circ}$C (Fig. \ref{figure:continent}). In Europe, such relationship is weak: advance \Sexpr{format(round(abs(mean(europeanpost_fitC_fall_lat)),digits = 2),nsmall=2)} (\Sexpr{format(round(abs(quantile(europeanpost_fitC_fall_lat, 0.95)), digits = 2),nsmall=2)} - \Sexpr{format(round(abs(quantile(europeanpost_fitC_fall_lat, 0.05)), digits = 2),nsmall=2)}) days per degree northward, or \Sexpr{format(round(mean(europeanpost_fitC_fall_mat),digits = 2),nsmall=2)} days (\Sexpr{format(round(abs(quantile(europeanpost_fitC_fall_mat, 0.05)), digits = 2),nsmall=2)} - \Sexpr{format(round(abs(quantile(europeanpost_fitC_fall_mat, 0.95)), digits = 2),nsmall=2)}) per degree decline in MAT $^{\circ}$C(Fig. \ref{figure:continent}). 
