...
Overall, spring events were not related to provenance latitude (insert the mean slope here, followed by the 90\% intervals) or mean annual temperature in North America (insert the mean slope here, followed by the 90\% intervals).  Spring events advanced slightly with provenance latitude and mean annual temperature in Europe (insert slope and UI), meaning spring events are slightly earlier where provenance mean annual temperature is lower (i.e., higher, more northern latitudes)(Fig. \ref{figure:springfall_latmat} \& Supplement Table. \ref{supplement-table:model_sf_lat}). Results were similar using other distance metrics in lieu of latitude (see Supplement Fig.\ref{supplement-figure:lat_distance} for results using the absolute value of the distance and the spherical distance difference between provenance and garden latitude). 
...
In contrast, fall events (budset, leaf senescence, leaf abcission) advanced strongly with provenance latitude and mean annual temperature, meaning fall events are earlier where provenance mean annual temperature is lower. This relationship, however, was observed mostly in North America where fall events advance \Sexpr{format(abs(median(northamericanpost_fitC_fall_lat)),digits = 2)} (UI = \Sexpr{format(abs(quantile(northamericanpost_fitC_fall_lat, 0.9)), digits = 2)}) days per degree we move north, or \Sexpr{format(median(northamericanpost_fitC_fall_mat),digits = 2)} days (UI) per degree decline in MAT$^{\circ}$C (Fig. \ref{figure:continent}). In Europe, such relationship is weak: advance \Sexpr{format(abs(median(europeanpost_fitC_fall_lat)),digits = 1)} (UI) days per degree we move north, or \Sexpr{format(median(europeanpost_fitC_fall_mat),digits = 1)} days (UI) per degree decline in MAT $^{\circ}$C(Fig. \ref{figure:continent}). 
