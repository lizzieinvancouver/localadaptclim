...
%7July2023: Add to this please! Add refs to figure and add the correct bibtex refs.
 \item Clines in fall phenology were stronger and more consistent whereas clines in spring phenology are weaker and somewhat varied in directionality. For fall events, only two field studies found no relationship (and ref Fig 2c-d; CITE Rosique-Esplugas 2021, Fraxinus excelsior from garden LETTER, which was in the UK and Petkova et al., 2017 Fagus sylvatica from garden LETTER, which was in Bulgaria, ), while another study was the only greenhouse experiment included (Rehfeldt 1994, also included Aitken and Bammals (2015)), which uniquely used the fall event of `the day by which seedling elongation had finished.'  Spring event clines were always weak, ... 
 %7July2023: Do any of the spring event clines for individual species x garden actually exclude 0 in their 90% interval? If not, add here:
 % (all species x garden clines included 0 in their 90\% intervals).
 % If so do not then add:
% ... were weak. Only XX and YY showed clines (give values with intervals) whereas the rest did not (all remaining species x garden clines included 0 in their 90\% intervals).
