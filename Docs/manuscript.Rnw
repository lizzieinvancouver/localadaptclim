...
%emw24Jan -- this section looks good; once you're happy with it, I can slim it down a little and suggest a few things to cut or move to supp. 

We searched the peer-reviewed literature for common garden experiments that documented the timing of spring events of woody plant species on 14 December 2022 using Web of Science (Thompson Reuters, New York, NY). Our search terms using all combinations of the following terms as ‘titles’ including ‘common garden*’ or “provenance*’ and ‘leafout*’ or ‘leaf out*’ or ‘budburst’ or ‘spring phenolog*’. The search returned 94 publications. %emw24Jan -- see edits that address your question. We can just give the last date we searched. Also, can you check your terms? It's not clear what the search was. ... I would just write it out as we have in other papers. Here's an example:
% TOPIC = (budburst OR leafout) AND (photoperiod or daylength) AND temperature*, which yielded 85 publications
\\
...
%emw24Jan -- I agree! You can say you also searched google scholar (but then you have to say how and when), and contacted authors of previous review papers to help further search the literature. The other thing you could do is your search with TOPIC instead of TITLE, as reviewers will find TITLE suspicious -- even though I suspect you have found all the main papers. 
...
%emw24Jan -- yes, we can move the below to modeling section
...
%emw24Jan -- I think we can delete this subsection (or move to supp for now). Just define clearly in caption what you mean. 
\subsubsection{Spherical distances}

hmmm need to better define this... 
On top of taking the difference between provenance latitude and garden latitude (maybe I could write this in subscript), we also use the dism() function in the Geosphere package in R to estimate the geospherical distance between locations (accounting for Earth curvature).\improvement{maybe there are different/better terms}
...
%emw24Jan -- the below is in the methods so not needed here. 
...
%emw24Jan -- below is good, can you also explain the direction of relationship in europe? Do events get later/earlier with higher/lower latitude and MAT?
Overall, our models show that spring events are not related to provenance latitude or mean annual temperature in North America, and only weakly related in Europe (Fig. \ref{figure:springfall_latmat} \& Supplement Table. \ref{supplement-table:model_sf_lat}). \unsure{Should I add climate overlap plots here too?} %emw24Jan -- do you mean add a table of climate overlap to the supp? 
